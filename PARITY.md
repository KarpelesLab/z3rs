...
| **QF_A / AX** | read-over-write + extensionality array axioms, **Bool-indexed arrays** (symbolic indices decided by exhaustive index case-split; constant indices by congruence) | array combinators (`map`, `as-array`) beyond folded forms |
| **QF_DT** | datatypes: enums, records, recursive & mutually-recursive, parametric; `match`, selectors/testers | — |
| **QF_S** | string constant folding, symbolic `str.len`, regex membership, length-link refutation, substring/prefix/suffix **monotonicity**, `str.at`/`str.substr`/`str.to_code`/`str.indexof` **length-range axioms**, length-guided bounded witness (on the pre-axiom goal, with abstract-model confirmation), and a **Nielsen word-equation procedure** refuting periodicity equations (`x·b=a·x`) | deep-content SAT word equations (length-coupled) |
