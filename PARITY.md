...
| **QF_S** | string constant folding, symbolic `str.len`, regex membership, length-link refutation, length-guided bounded witness, and a **Nielsen word-equation procedure** refuting periodicity equations (`x·b=a·x`) | deep-content SAT word equations (length-coupled) |
...
| **Quantified UF/LIA** | ground instantiation to a fixpoint, E-matching (recursive functions), finite Datalog/CHC, **Skolemization of `∃` under `∀`** (refutes `∀x∃y. y>x∧y<x`), **fresh-seed refutation** of un-grounded universals (`∀x. p(x)∧¬p(x)`), and **linear-real `∀x∃y` by nested QE** (both directions) | integer `∀∃` (needs Cooper), nonlinear `∀∃`, MBQI sat over uninterpreted functions |
