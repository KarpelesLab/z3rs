...
| **QF_LIA** | Fourier–Motzkin + branch-and-bound + Omega-test (GCD tightening, dark-shadow witness), **recursive disequality case-split into the dark shadow** (`distinct x y z`, n-way), free-variable-disequality elimination, **integer `v = const` inline** (so lifted-`mod` goals like `mod(x+y,3)=0 ∧ mod(x,3)=1 ∧ mod(y,3)=1` refute by GCD), **integer range pigeonhole** (`distinct v0…v3 ∧ ∀i. 0 ≤ vi ≤ 2` → unsat, `n` distinct integers can't fit a range of size `< n`) | instances exceeding the work budget |
...
| **QF_A / AX** | read-over-write + extensionality array axioms, **extensionality refutation from a universal** (`a≠b ∧ ∀i. a[i]=b[i]`), **Bool-indexed arrays** (index case-split), **const-array binding inline**, **`(_ map f)` equalities by pointwise expansion**, **bit-vector arrays read-over-write**, **store-equality witness** (`store a i c = store b i c` decides sat via the candidate model `a = b`) | non-constant array combinators beyond the read set |
| **QF_DT** | datatypes: enums, records, recursive & mutually-recursive, parametric; `match`, selectors/testers; **occurs-check refutation of cyclic variable equalities**, **enum pigeonhole** (`distinct v0…v4` over a 4-value enum), **nested ground selector folding** (`v(l(l(node(node(leaf 1,leaf 2),leaf 3)))) = 9` and `t = node(…) ∧ v(l t) = k` both refute, via `dt_fold` in the ground-binding inline + a whole-goal unsat-only fallback) | *performance* on a selector over a **variable** bound to a ground constructor (`t = node(…) ∧ v(l(l t))`) — fully-ground selector towers now fold in `check_sat` and decide fast |
| **QF_S** | string constant folding, symbolic `str.len`, regex membership, length-link refutation, substring/prefix/suffix **monotonicity**, `str.at`/`str.substr`/`str.to_code`/`str.indexof` **length-range axioms**, **`str.contains ⟺ str.indexof …0 ≥ 0`**, **disjoint/overlap multi-`contains` length bounds**, **`str.to_int` digit witness + `len ≥ #digits` bound**, **`str.to_int∘str.from_int` round-trip fold**, **`str.<` / `str.<=` order theories**, **`str.replace` (concrete s,t) → concat**, **congruent `str.at x i` markers**, **prefixof/suffixof against a literal at a fixed length pins `x`** (`prefixof x "abcd" ∧ suffixof x "abcd" ∧ len x = 2` refutes), **suffixof pins the last char at `len x−1`**, **`contains x P` length axioms** (`len x ≥ len P`; `len x = len P ⇒ x = P`), **regex ∩ predicate character conflicts** (`x ∈ (a-c)+ ∧ prefixof/suffixof/contains/`str.at` a char outside the language's begin/end/alphabet` → unsat), **bounded disjunctive case-split of the concat split** (so a pin reaches `str.to_int`/`str.at`/`str.len` under a disjunction — `x·y = "123" ∧ str.to_int x = 99 ∧ len x = 2` refutes), length-guided bounded witness, and a **Nielsen word-equation procedure** | deep-content SAT word equations (length-coupled); full length-aware regex automaton product |
| **QF sequences** | integer-sequence length links, additive `seq.++` length with **part-length non-negativity** (`s ++ u = empty ∧ len s = 2` refutes), congruent `seq.nth`, **concrete-sequence inline with re-fold**, **concat split at a pinned `len s`** (`s ++ u = [1,2] ∧ len s = 1 ∧ nth u 0 = 9` refutes), **`seq.extract` element axioms** (`nth(extract s i l, j) = nth(s, i+j)` + clamped length) and **concrete-sequence equality pinning** (`A = [c0,…] ⇒ len A = n ∧ nth(A,j) = cj`, so `extract s 1 1 = unit 3 ∧ nth s 1 = 2` refutes), and a **bounded SAT witness** deciding concat/`nth`/empty-subsequence cases | symbolic element sorts beyond Int; long sequences past the search bound |
...
| **QF_NIA** | linearization, univariate CAD, bounded-integer search + **integer-witness sampling**, **≤2-variable box witness** (difference-of-squares `x²=y²+17`, small solutions) and **bounded-region unsat**, **interval-coefficient product bounds** (factoring `x·y=7`; coupled `x·y=z ∧ y·z=x`), **product cancellation** (`x·y = k·x ⇒ x=0 ∨ y=k`, incl. repeated factors `x·x = k·x`), **difference of squares** (`a² = b² ⇒ a=b ∨ a=−b`), **zero-product** (`x·y=0 ⇒ x=0 ∨ y=0`), **sum-of-squares** (`Σ aᵢ²=0 ⇒ ⋀ aᵢ=0`), **square monotonicity** (`0≤a<b ⇒ a²<b²`), **bounded-product enumeration** (every product variable pinned to a small range is enumerated, so `(x²=4 ∨ x²=9) ∧ 0<x<2` and `1≤x≤3 ∧ x²∈{14,20}` refute soundly instead of a spurious sat), variable elimination, **symbolic div/mod** (compound divisors, stable-tail UNSAT, `div(t,t)`/`mod(t,t)`) | undecidable/unbounded nonlinear integer cases beyond the box |
...
| **Theory combination** | **array reads of a free array as free variables** into the nonlinear engine and the bit-blaster (**arrays of bit-vectors**, read-over-read/write), **Ackermannization of uninterpreted BV functions** (`∀x. g(x)=x+1 ∧ g(k)=0`), and **`to_fp(to_real n)` re-folded when `n` is pinned** (re-blasting the FP equality) | reads coupling ≥2 theories through a stored/aliased array |
...
Across the project this method drove out **20+ real soundness bugs**, each fixed
...
- an FP `ite` blasted to a fresh, unconstrained bit-vector, so the selector never
  tied the result to the chosen branch (`fp.eq(ite(b,x,+0),+0) ∧ b ∧ isNormal x`
  reported `sat` not `unsat`) — now blasted to a bit-vector `ite` of the branches
...
Since this table was last regenerated, further refutation capability landed
(regex ∩ predicate character conflicts, nested ground datatype selectors, array
store-equality witness, the concat-split disjunctive case-split, NIA product
cancellation / difference-of-squares, integer range pigeonhole, `seq.extract`
element axioms, `str.contains` length axioms) — each validated at 0 unsound on its
own targeted fuzz. See [`ROADMAP.md`](ROADMAP.md) §7 for the tracked remaining
tail.
