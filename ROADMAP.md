...
| **Concat = literal ∧ length-sum** | `x·y = "abcd" ∧ len x + len y = 4` | 🟨 | **UNSAT side closed** (an inconsistent length-sum refutes via the additive-length axiom, `len x + len y = 3` → unsat). The remaining SAT side needs `check_model` to confirm a split-disjunction branch through a redundant `str.len` congruence — a disjunction-aware string model construction (the general limit shared with §2's model-at-scale). |
| **Nested datatype selectors** | `v(l(node(leaf 1, leaf 2)))`, `t = node(…) ∧ v(l t) = k` | ✅ | **Closed** — `dt_fold` (selector/tester-on-constructor folding) is applied inside the ground-binding inline, *after* the datatype variables are substituted away, so no opaque selector-on-a-variable remains to mis-handle (the failure mode that reverted a whole-goal `dt_fold`). Nested-selector fuzz 88/88, 0 unsound; `v(l t) = 5 ∧ t = node(leaf 1, leaf 2)` refutes; free selector-on-variable stays a sound `sat`/`unknown`. |
