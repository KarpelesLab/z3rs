...
| G | Horn (Spacer/PDR) | 🟨 | single-predicate CHC now robust: BMC + k-induction, **safety-property heads** (`P(x) ⇒ prop(x)`), **non-bare predicate args** (`inv(x+1)` via equality normalisation), tester exclusion — fuzz 149/149, 0 unsound. Remaining: **multi-predicate** systems (`p → q` chains, recursion) need the symbolic-reachability / PDR engine with model-based projection (z3's most complex component) — specced (`spacer_multi_spec`), not built |
