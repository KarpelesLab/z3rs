...
| 5     | `smt` / `nlsat`              | 🟨     | lazy DPLL(T): congruence-closure e-graph (QF_UF, congruence at every sort) + Fourier–Motzkin linear arithmetic with witness reconstruction (QF_LRA) + integer branch-and-bound with a gcd/divisibility test (QF_LIA); Nelson–Oppen sharing of arithmetic-implied equalities (QF_UFLRA/QF_UFLIA); satisfying-model extraction; sound `unknown` when B&B is inconclusive · to do: complete integer procedure, EUF→arith equality sharing, simplex, online propagation, bv/array theories, quantifiers |
| 6     | `solver` / `cmd_context`     | 🟨     | SMT-LIB2 front end: declares, assert, check-sat, get-value, get-model, let, push/pop/reset, define-fun, Bool/Int/Real, linear arith + UF, distinct, div/mod/abs/to_real/to_int (Euclidean, constant-folded), term-ITE lifting; `z3rs file.smt2` decides QF_UF/QF_LRA/QF_LIA, reports models, and is differential-tested against z3 (`tests/differential.rs`) · to do: get-unsat-core/assumptions, get-info responses, full command set |
| 7     | `qe` / `muz` / `opt`         | ⬜     |       |
| 8     | `parsers`                    | ⬜     |       |
| 9     | `api`                        | ⬜     |       |
| 10    | hardening & parity           | ⬜     |       |
