...
| 5     | `smt` / `nlsat`              | 🟨     | lazy DPLL(T): congruence-closure e-graph (QF_UF, congruence at every sort) + Fourier–Motzkin linear arithmetic with witness reconstruction (QF_LRA) + integer branch-and-bound (QF_LIA); satisfying-model extraction · to do: complete integer procedure (B&B is best-effort on unbounded), Nelson–Oppen implied-equality sharing, simplex, online propagation, bv/array theories, quantifiers |
