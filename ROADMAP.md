...
| C | floating-point theory | 🟨 | non-arithmetic surface done (`abs`/`neg`, exact `to_real`, fast ordered compares, `min`/`max`, `eq`+classification); **bit-exact `fp.add`/`sub`/`mul`/`div`/`sqrt`** (ports of z3's `mk_*`, all 5 rounding modes, fuzz 0 unsound); **`to_fp(real/int)` folded to any format/mode**, so concrete FP arithmetic decides. Remaining: `fma`/`rem`/`roundToIntegral`/`to_ubv`/`to_sbv`, and Float32/64 *symbolic* circuits (too large for the CDCL — performance-bound like QF_BV, ⇒ Phase A) |
