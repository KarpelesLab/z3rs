...
| 7     | `qe` / `muz` / `opt`         | 🟨     | **opt**: `(maximize)`/`(minimize)`/`(get-objectives)` for integer objectives — binary-search optimization (doubling bracket + bisection) over the LIA solver, lexicographic across objectives, `oo`/`(- oo)` for provably-unbounded, sound `unknown` otherwise; plus **`(assert-soft)` weighted MaxSAT** (penalty-variable encoding minimized ahead of user objectives); matches z3's output · to do: real-valued objectives (strict-bound simplex), quantifier elimination (`qe`), Horn/Datalog fixed-point (`muz`/Spacer) |
