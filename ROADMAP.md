...
| 5     | `smt` / `nlsat`              | 🟨     | lazy DPLL(T): congruence-closure e-graph (QF_UF) + Fourier–Motzkin linear arithmetic with witness reconstruction (QF_LRA) + integer branch-and-bound (QF_LIA); satisfying-model extraction · to do: Nelson–Oppen combination, simplex, online propagation, bv/array theories, quantifiers |
| 6     | `solver` / `cmd_context`     | 🟨     | SMT-LIB2 front end: declares, assert, check-sat, get-value, get-model, let, push/pop/reset, define-fun, Bool/Int/Real, linear arith + UF; `z3rs file.smt2` decides QF_UF/QF_LRA/QF_LIA and reports models · to do: get-unsat-core/assumptions, get-info responses, full command set |
| 7     | `qe` / `muz` / `opt`         | ⬜     |       |
| 8     | `parsers`                    | ⬜     |       |
| 9     | `api`                        | ⬜     |       |
| 10    | hardening & parity           | ⬜     |       |
