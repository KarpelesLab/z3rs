...
| **Regex ∩ predicate** | `x ∈ (a-z)+ ∧ suffixof "d" x`; `re.comp`/`re.inter`/`re.loop` coupled to `contains`/length | 🟨 | **First-character conflict closed** (`x ∈ (a-c)+ ∧ prefixof "d" x` → unsat, via a sound `can_start_with` over-approximation). Remaining: last-character/`suffixof`, interior `contains`, and a full length-aware automaton product. |
