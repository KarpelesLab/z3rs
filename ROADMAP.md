...
| B | symbolic `div`/`mod` | 🟨 | comprehensive: SAT-witness (zero divisor + goal-derived candidates), constant-dividend **stable-tail UNSAT** decision, **compound divisors** via a fresh alias (`dv=x+y`), and `div(t,t)=1`/`mod(t,t)=0`. Fuzzer gap **45%→~1.3%**, 0 unsound. Residual: proportional non-unit ratios (`div(y,2y)`), pathological coupled `div((x+3),(x+y))` (slow) |
