...
| **Regex ∩ predicate** | `re.comp`/`re.inter`/`re.loop` coupled to `contains`/length | 🟨 | **Character-conflict cases closed**: `str.in_re x r` vs `prefixof`/`suffixof`/`contains`/`str.at x 0` refutes when the required character is outside what the language can begin/end/contain (sound `can_start_with`/`can_end_with`/`can_contain` over-approximations). Remaining: a full length-aware automaton product (`re.loop` with specific lengths, positional constraints beyond the first/last char). |
| **Concat = literal ∧ length-sum / predicate** | `x·y = "abcd" ∧ len x + len y = 4`; `x·y = "123" ∧ str.to_int x = 99 ∧ len x = 2` | ✅ | **Closed** by a bounded disjunctive case-split: the concat split `⋁ₖ (x = lit[:k] ∧ …)` is split against the other conjuncts and each branch re-decided through the full pipeline, so a pin propagates into `str.to_int`/`str.at`/`str.len`/`suffixof` that `check_model` left opaque under a disjunction. Sound (`(⋁ b) ∧ R` sat iff some `b ∧ R` sat), gated to string-pinning disjunctions of ≤ 12 branches, terminating (each level removes the split OR). Fuzz 124/124, 0 unsound, no slowdown. |
| **Nested datatype selectors** | `v(l(node(leaf 1, leaf 2)))`, `t = node(…) ∧ v(l t) = k` | ✅ | **Correctness closed** — `dt_fold` folds selector/tester-on-constructor chains inside the ground-binding inline (after variables are substituted) and in a whole-goal fallback that takes only `unsat` (so a residual opaque selector-on-a-variable is never trusted `sat`). Fuzz 117/117, 0 unsound. · *Perf*: **fully-ground** nested selectors (`v(l(l(node…))) = 9`) now fold before the axioms in `check_sat` and decide in ~0s (was seconds). The residual tail is a selector over a **variable** bound to a ground constructor (`t = node(…) ∧ v(l(l t)) = k`) — the eager eta/selector axioms still unfold over `l t`; bounding that is the open perf work. |
