...
| **Nested datatype selectors** | `v(l(node(leaf 1, leaf 2)))`, `t = node(…) ∧ v(l t) = k` | ✅ | **Correctness closed** — `dt_fold` folds selector/tester-on-constructor chains inside the ground-binding inline (after variables are substituted) and in a whole-goal fallback that takes only `unsat` (so a residual opaque selector-on-a-variable is never trusted `sat`). Fuzz 117/117, 0 unsound. · *Perf tail*: on a **recursive** datatype the eager eta/selector axioms unfold with the selector-nesting depth (a small `v(l(l(node…)))` takes seconds), decided but slow — bounding that unfolding is the open perf work. |
