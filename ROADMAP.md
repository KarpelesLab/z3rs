...
| 6     | `solver` / `cmd_context`     | 🟨     | SMT-LIB2 front end: declares, assert, check-sat(-assuming), get-value, get-model, get-unsat-core, echo/get-info, let, push/pop/reset(-assertions), define-fun, named assertions; Bool/Int/Real + `(Array I E)` + `(_ BitVec n)`; linear arith + UF + arrays + bit-vectors, distinct, div/mod/abs/to_real/to_int/is_int (Euclidean, constant-folded), term-ITE + array-axiom lifting; `z3rs file.smt2` decides QF_UF/QF_LRA/QF_LIA/QF_A/QF_BV, reports models/cores, differential-tested against z3 (`tests/differential.rs`) · to do: bv models, remaining command set |
| 7     | `qe` / `muz` / `opt`         | ⬜     |       |
