...
| 9     | `api`                        | 🟨     | **C ABI** (opt-in `ffi` feature → static/shared lib, only `unsafe` module): `z3rs_eval_smtlib2_string` (mirrors `Z3_eval_smtlib2_string`), `z3rs_version`, `z3rs_string_free`; C header `include/z3rs.h`, C smoke test + CI `c_abi` job; dependency-freedom preserved (`cargo tree` stays `z3rs → puremp`) · to do: the full `z3_api.h` handle surface (contexts, ASTs, solvers, models, tactics), Rust safe API |
| 10    | hardening & parity           | ⬜     |       |
