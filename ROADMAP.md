...
| 5     | `smt` / `nlsat`              | 🟨     | lazy DPLL(T): congruence-closure e-graph (QF_UF, congruence at every sort incl. Bool-valued predicates) + Fourier–Motzkin linear arithmetic with witness reconstruction (QF_LRA) + integer branch-and-bound with gcd/divisibility test and strict-inequality tightening (QF_LIA) + read-over-write array axioms (QF_A); bidirectional Nelson–Oppen equality sharing (QF_UFLRA/QF_UFLIA); satisfying-model extraction; work-budgeted search with sound `unknown`; differential-fuzz-hardened against z3 · to do: complete integer procedure, simplex, online propagation, bv theory, extensional/quantified arrays, quantifiers |
| 6     | `solver` / `cmd_context`     | 🟨     | SMT-LIB2 front end: declares, assert, check-sat(-assuming), get-value, get-model, get-unsat-core, let, push/pop/reset(-assertions), define-fun, named assertions; Bool/Int/Real + `(Array I E)`; linear arith + UF + arrays, distinct, div/mod/abs/to_real/to_int (Euclidean, constant-folded), term-ITE + array-axiom lifting; `z3rs file.smt2` decides QF_UF/QF_LRA/QF_LIA/QF_A, reports models/cores, differential-tested against z3 (`tests/differential.rs`) · to do: get-info responses, remaining command set |
| 7     | `qe` / `muz` / `opt`         | ⬜     |       |
| 8     | `parsers`                    | ⬜     |       |
| 9     | `api`                        | ⬜     |       |
| 10    | hardening & parity           | ⬜     |       |
