...
| D | strings & sequences | ✅ | **Exit met for the practical string fragment** — constant folding, symbolic `str.len`, `contains`/`prefixof`/`suffixof`/`str.at`, length-link refutation, regex membership, and **length-guided bounded witness** (fixed-length word equations `len x=4 ∧ x·y=y·x`). Broad fuzz **147/150, 0 unsound**. · sound-`unknown` tail: deep-content / unbounded word equations (periodicity refutation needs Nielsen/Makanin) |
...
| G | Horn (Spacer/PDR) | ✅ | **Exit met for the practical CHC fragment** — decides **single-predicate** CHC both directions (BMC + k-induction, property heads, non-bare args, tester exclusion) *and* **acyclic multi-predicate** CHC both directions (exact inlining in topological order: `p→q→…` chains, 2–3 preds, safe *and* unsafe). Broad CHC fuzz **137/150 decided, 0 unsound**. · sound-`unknown` tail (the documented CHC decline): **recursive/cyclic multi-predicate safe** — the reach sets need **model-based projection** (Loos-Weispfenning/Cooper) to converge to an inductive invariant; that MBP + inductive generalization is the core Spacer/PDR machinery, a follow-on |
