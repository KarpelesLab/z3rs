...
| 5     | `smt` / `nlsat`              | 🟨     | lazy DPLL(T): congruence-closure e-graph (QF_UF, congruence at every sort incl. Bool-valued predicates) + Fourier–Motzkin linear arithmetic with witness reconstruction (QF_LRA) + integer branch-and-bound with gcd/divisibility test and strict-inequality tightening (QF_LIA); bidirectional Nelson–Oppen equality sharing (QF_UFLRA/QF_UFLIA); satisfying-model extraction; node-bounded B&B with sound `unknown`; differential-fuzz-hardened against z3 · to do: complete integer procedure, simplex, online propagation, bv/array theories, quantifiers |
