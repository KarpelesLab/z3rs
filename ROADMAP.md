...
| C | floating-point theory | 🟨 | non-arithmetic surface done; **bit-exact `fp.add`/`sub`/`mul`/`div`/`sqrt`/`roundToIntegral`** (ports of z3's `mk_*`, all 5 rounding modes, fuzz 0 unsound); **`to_fp(real/int)` folded to any format/mode**. **Concrete FP arithmetic decides for all formats (Float16/32/64), fast**; symbolic Float16 decides. Remaining: `fma` (390-line port), `rem` (230-line), `to_ubv`/`to_sbv`, `to_fp(fp→fp)`; and Float32/64 *symbolic* circuits (too large for the CDCL — performance-bound like QF_BV, ⇒ Phase A) |
